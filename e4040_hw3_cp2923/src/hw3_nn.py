...
class LeNetConvPoolLayer(object):
    ...
    def __init__(self, rng, input, filter_shape, image_shape, poolsize, border_mode='half'):
        """
        ...
        self.forinput = T.nnet.relu(conv_out + self.b.dimshuffle('x', 0, 'x', 'x'))
...
class BNLeNetConvPoolLayer(object):
    """Pool Layer of a convolutional network """
    ...
    def __init__(self, rng, input, filter_shape, image_shape, poolsize, border_mode = "half"):
        ...
        out = self.BNlayer.get_result(conv_out)

        bn_out = T.switch(out<0, 0.01*out, out)

        if poolsize == None:
            self.output = T.nnet.relu(bn_out + self.b.dimshuffle('x', 0, 'x', 'x'))
        # pool each feature map individually, using maxpooling
        else:
            pooled_out = pool.pool_2d(
                input=bn_out,
        ...
        # store parameters of this layer
        self.params = [self.W] + self.BNlayer.params
