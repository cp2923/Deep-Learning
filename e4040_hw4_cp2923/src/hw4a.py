...
class RNNSLU2(object):
    ...
    def __init__(self, nh, nh2, nc, ne, de, cs, normal=True):
        ...
        # bundle
        self.params = [self.emb, self.h0, self.wx, self.wh, self.bh, self.h2, self.wx2, self.wh2, self.bh2, self.w, self.b]
        ...
        def recurrence(x_t, h_tm1,h2_tm1):
            ...
            h2_t = T.nnet.sigmoid(T.dot(h_t,self.wx2) + T.dot(h2_tm1, self.wh2)+ self.bh2)
            s_t = T.nnet.softmax(T.dot(h2_t, self.w) + self.b)
            return [h_t, h2_t, s_t]

        [h,h2,s], _ = theano.scan(fn=recurrence,
                               sequences=x,
                               outputs_info=[self.h0, self.h2, None],
                               n_steps=x.shape[0])
...
def similarity(model, idx, idx1, idx2, idx3, idx4):
    x1 = model.emb.get_value()[idx1,]-model.emb.get_value()[idx2,]
    x2 = model.emb.get_value()[idx3,]-model.emb.get_value()[idx4,]
    sim = x1.dot(x2)
    print('The similarity between (' + idx[idx1] + ' - ' + idx[idx2] + ') and ('+ idx[idx3] + ' - ' + idx[idx4] + ') is: ' + str(sim))
